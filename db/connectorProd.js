...
export async function connectorProd(statement) {
    const database = 'prod'
    const login = process.env.loginIfs
    const haslo = process.env.hasloProd
    const connectString = process.env.connectStringProd
      let connection;
      
    try {
        const pool = await initializePool(database,login, haslo, connectString);
        const poolAlias = pool.poolAlias; 
        connectorProd.poolAlias = poolAlias;
