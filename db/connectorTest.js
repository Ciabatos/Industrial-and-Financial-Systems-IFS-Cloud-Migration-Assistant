...
export async function connectorTest(statement) {
    const database = 'test'
    const login = process.env.loginIfs
    const haslo = process.env.hasloTest
    const connectString = process.env.connectStringTest
      let connection;
      
    try {
        const pool = await initializePool(database,login, haslo, connectString);
        const poolAlias = pool.poolAlias; 
        connectorTest.poolAlias = poolAlias;
