...
export async function connectorCfg(statement) {
    const database = 'cfg'
    const login = process.env.loginIfs
    const haslo = process.env.hasloCfg
    const connectString = process.env.connectStringCfg
      let connection;
      
    try {
        const pool = await initializePool(database,login, haslo, connectString);
        const poolAlias = pool.poolAlias; 
        connectorCfg.poolAlias = poolAlias;
